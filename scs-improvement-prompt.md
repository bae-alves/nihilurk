# Prompt: retiring the weak and the ugly

A session prompt for the SCS punch list — the seven items in
`scs-architecture/03-principles-and-manifesto.md#improvement-path`.

**How to use it.** Start a fresh session at the repo root and say "read
`scs-improvement-prompt.md` and follow it". Set MODE first.

---

## MODE — edit this line before you start

    MODE = DESIGN

* `DESIGN` — produce the plan, stop at the checkpoint. No code edits, no new files but the plan.
* `DESIGN+P1` — produce the plan, stop at the checkpoint, then implement Phase 1 only.

If that line is missing or unreadable, treat it as `DESIGN` and say so in one
line before you begin. The mode never escalates on its own: nothing past Phase 1
is implemented without a fresh go-ahead, whatever the mode says.

---

## ROLE

You are a staff-level Rust engineer who has shipped simulation-heavy games and
...
describe roog; they do not govern it. Where a change would make the game better
...
item that makes the doctrine tidier without making roog better to work on is a
...
**roog is an engine that displays and resolves content, and the content is a set
...
(`docs/explanation/ecs-in-roog.md:31`).
...
no pink dragon in roog. This is the whole of putting one in" — one line, then
...
* **The ratio call** — value over cost, stated as a verdict: *do it now*, *do it when X*, or *decline*. Not a score, not a paragraph. A phase without one is unfinished. Price both sides in roog's currency, never in hours and never in lines touched:
  ...
  * *what has to stay true by hand* — a new invariant nobody checks is a cost that accrues interest.
  Value is always to a named person doing a named thing to roog. "Cleaner" is not a value. Ignore the pride easter egg always.
* **Green** — `cargo fmt`, `cargo build`, `cargo test`, `cargo clippy`, all four clean at the workspace root, with `models/tests/determinism.rs` among the tests that pass. Also, run the perf and compat pipelines before touching code and after, so you have a point of comparison.
...
* **The project.** roog: a terminal roguelike in Rust. ~24k lines of source across four shipped crates, ~35k with the test suite. ECS, data-driven content, and a frankly imperative, mutation-heavy turn loop. Ships as a small static musl binary.
...
* **The name.** SCS, the Simulationist Component System. `scs.md` is the essay; `scs-architecture/` is the generalised five-file set.
...
|---|---|---|
| 1 | `scs-architecture/02-strong-weak-ugly.md` | the self-review this punch list answers |
| 2 | `scs-architecture/03-principles-and-manifesto.md` | the seven items, in their original wording |
| 3 | `docs/explanation/ecs-in-roog.md` | why almost every system takes `&mut World` |
...
manifesto is the one being executed. Do ask me when they differ with suggested courses of action, though.
...
Two of these are not the items they look like. Item 1 is mostly built already,
and the third of it that is not is hard for a reason no wording here conveys;
item 5 is already answered in the code. Read GROUND TRUTH before forming a view
on either.
...
Verified at `19b8bce` on 2026-09-15. Numbers drift: re-run the block below and
...
| | |
|---|---|
| Workspace | `engine/` (main loop, input, render), `models/` (the simulation), `view/`, `particle-core/`; `perf/` and `compat/` are rigs, excluded from `default-members` |
| Schedule | `engine/src/main.rs:381-417` — sixteen systems, every `.after()` edge commented and load-bearing. Fifteen take `&mut World`; `visibility_system` is the lone `Query`-style one. |
| Turn loop | `engine/src/main.rs` main loop; `engine/src/update.rs` owns the keyboard and the modal stack |
| Content tables | `models/src/catalog.rs` (ten tables), `models/src/monsters.rs` (bestiary), traps in `models/src/traps.rs` |
| Effect vocabulary | `models/src/effects.rs` — markers, modifiers, caps, and `Grant` |

**The numbers that motivate the punch list.** Run this block first:

```sh
S="engine/src models/src view/src particle-core/src"
rg -o "fn \w+[^;{]*&mut World" -g '*.rs' $S | wc -l          # 356  the imperative surface
rg -o "fn \w+[^;{]*&World\b"   -g '*.rs' $S | rg -vc "&mut"  #  47  the read-only half
rg -o "GameLog>\(\)"           -g '*.rs' $S | wc -l          # 235  item 1: side effects decided inline, everywhere
rg -o "Particles>\(\)"         -g '*.rs' $S | wc -l          #  44  same, for the feel layer
rg -o "Shake>\(\)"             -g '*.rs' $S | wc -l          #   7  ditto
rg -oi "assumes|invariant|must run after" -g '*.rs' $S | wc -l  # 0  item 2 is at zero in the systems themselves
rg -c "#\[test\]" models/tests/content.rs                    #  16  item 3 is half-built
rg -o "\.despawn\(" -g '*.rs' $S | wc -l                     #  24  item 4 is better than it looks
rg -o "world\.spawn\(" -g '*.rs' $S | wc -l                  #   7  ditto
rg -o "iter_entities\(\)" -g '*.rs' $S | wc -l               #   3  item 5's whole population
wc -l engine/src/update.rs models/src/catalog.rs             # 2123 / 1207  the two biggest files
```

### Three of the seven are already half-built

Complete them. Do not build a parallel system beside them.

* **Item 1 (queues).** Two thirds of this is already built, in two different places, and the raw counts above hide it.
  * *Inbound — done.* Four intent queues in `models/src/components.rs:1015-1049`: `AttackQueue`, `UseQueue`, `ThrowQueue`, `MoveQueue`, each a `Vec` resource filled by input/AI and drained by one named system.
  * *Outbound cosmetic — done.* `docs/explanation/the-feel-layer.md` already states the rule as **"gameplay arms a flourish and forgets it"**. `fx.hit_spark(x, y)` and `shake::kick_shake(world, ShakeKind::Heavy)` are named arms that never wait, never check and never read back, reached through `get_resource_mut` so a headless world runs the same path. The 44 `Particles` and 7 `Shake` sites are that pattern in use, not a mess to tidy. Do not refactor them; copy them.
  * *Outbound narrative — open.* The 235 `GameLog` sites are the actual item, and they are the outlier for a reason that is not style. See step 4.
* **Item 3 (validation).** `models/tests/content.rs` exists. What it lacks is field-level legality and impossible-combination checks *within* a row. Also, check if the manual is still consistent and add in mechanics that are not in Rogue, using Rogue's manual as a base: 'https://britzl.github.io/roguearchive/files/misc/EpyxRogueDOSManual/manual.htm'.
* **Item 5 (indexes).** Read `models/src/equipment.rs:125-131` and `docs/explanation/ecs-in-roog.md:110` before proposing anything. Between them they settle it: the full-world scan is a constraint rather than a choice (gear points at its wearer; every caller holds `&World` mid-read), it is cheap on a floor of tens of entities, and *"if a floor ever held thousands, the fix is an index resource, not a smaller loop"*. Do not implement it. State the entity-count threshold at which that argument stops holding, and how we would notice crossing it. "Not yet, and here is the trigger" is a complete answer.

### Item 7, already happening

`docs/explanation/ecs-in-roog.md:9-10` and `:75` say the schedule has thirteen
steps, twelve of them `&mut World`. It has sixteen, fifteen. Nothing is broken;
the doc stopped tracking the code. That is what "docs at the seam" has to
prevent: a contract that quietly stops being true. Fold the correction into
whichever phase touches that doc and ensure pre-commit will not allow any commit
that doesn't also touch any markdown in this base.

The same page drifts twice. `:35` calls them "the three intent queues"; there
are four, in `models/src/components.rs:1015-1049`. One stale number is an
oversight. Two in one file is the argument for item 7 making itself.
...
2. roog's quality, over SCS's tidiness. The essay describes the game; amend the essay.
...
* **Generic.** Advice that would read the same against any Rust ECS project. The test is mechanical: does the sentence contain a roog path? If it does not, it is an essay, not a plan.
...
* **Doctrine first.** A phase that exists so SCS reads better. The test: name the person it helps and what they were trying to do to roog when it helped them. If the answer is "a reader of `scs.md`", cut the phase.
...
* **Phase 4: validation and debug tooling** — roog already ships some: `cargo run -p engine -- -content` prints every name the game knows, and `ROOG_SPAWN="pink dragon"` puts one in front of you. Extend those; a parallel debug surface is a second thing to keep true.
...
Write the plan to `scs-improvement-plan.md` at the repo root.
...
1. Every claim about roog's code carries a path, and every claim that needs a line number has one.
...
5. Every phase names who it helps and what they were trying to do to roog. No phase is there to make `scs.md` read better.
...
* **Docs ship with it.** The house rule is not suspended because the commit is deferred. Every page the change makes untrue is corrected in the same working tree — including the schedule counts in `docs/explanation/ecs-in-roog.md` if this is the phase that touches them.
