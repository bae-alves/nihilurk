...
/// named fixed files in the shared temp directory (`roog_test.sav`,
/// `roog_melee_cap.sav`, …), so two `cargo test` runs at once — or a CI matrix
...
impl SaveFile {
    ...
    pub fn new(tag: &str) -> Self {
        Self(std::env::temp_dir().join(format!("roog-{}-{tag}.sav", std::process::id())))
    }
