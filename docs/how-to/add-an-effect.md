...
An effect is a component on the creature, and both the dragon born
fire-immune and the player wearing a ring of fire resistance carry the
same one. (Why that is the whole design:
`../explanation/data-driven-content.md`.)
...
Markers are the common case. Take that branch unless your property is
literally an integer that adds up, or a limit that overrides one.

There is one cap today (`MeleeCap`, carried by bows and crossbows) and it
is read by one line of `resolve_attack`. A second would be the moment to
generalise the fold the way `equipped_total` is generalised over
`Modifier`; see `melee_cap` in `models/src/effects.rs`.


Adding a marker effect
----------------------

1. **Declare the component** in `models/src/effects.rs`, next to its
   relatives. It must be a unit struct deriving exactly this set:
...
That is the whole of it. Equipping and unequipping, saving and loading,
and the wand of cancellation all work immediately, because all three walk
`EFFECTS` rather than a list of special cases.
...
Rarer, and a bigger change, because a number has to be added *somewhere*
specific -- there is no generic place a new number belongs.

1. Declare it with the `modifier!` macro in `effects.rs`:
   ...
   This gives you `pub struct SightBonus(pub i32)` implementing
   `Modifier`.

2. Attach it from rows the way `ArmorBonus` is attached -- see
   `RingDef::armor_bonus` and `insert_modifier`, which skips a zero so an
   inert row costs nothing.

3. **Fold it where it applies**:

       equipped_total::<SightBonus>(world, entity)

   That sums the component across everything the entity has equipped plus
   anything it carries itself. Call it from the system that cares. This
   is the step no framework can do for you: `PowerBonus` is folded in
   combat, `ThrowBonus` in the throw code, and your new one has to be
   folded by whoever owns the number it modifies.

Modifiers are not in `EFFECTS` and are not bits in a save; they are saved
...
If your effect should survive a reload, the test worth copying is in
`models/tests/wands.rs`: the one named
`cancellation_strips_the_magic_but_leaves_the_creature` exercises the
grant / probe / revoke path end to end.
