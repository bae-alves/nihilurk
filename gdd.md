...
From floor 6 to floor 12, one floor in ten is not Rogue's grid of rooms at all. A battlefield is one room wall to wall, the whole floor in view, with twice the monsters and twice the loot. A labyrinth is a maze of passages with a few loops in it, and dark all the way through. A vault is a white honeycomb of rooms, each joined to its neighbours by one door, with the stairs in two of its corners, twice the monsters and three times the treasure. The bee world is one yellow cave, every monster in it an apis, a budget of them for each difficulty tier, and three times the treasure. A castle says nothing when you arrive: it is an ordinary floor with a 7x7 keep in the middle, a dragon inside, and a walled tower on each corner, all five stocked like whole floors. An island is land in deep water with the whole sea in view; only swimmers go in the water, and the eels in it breathe lightning. Whatever lands in the water sinks with a splash, except the Element of Yoord, which will not drown: it leaps back into your hands and burns away everything else you carry to make room. On the floors that are one room and nothing else, the ground around the stair you arrive on is kept clear, the way a start room is. Rooms that share a wall light up one at a time, since a door shows the first step into the next room and no more. A special floor never has special rooms, and because it is part of the layout, it is still special on the way back up. No floor puts a special room around a staircase.
